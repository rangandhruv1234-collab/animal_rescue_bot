...
ADMIN_NUMBER    = os.getenv("ADMIN_NUMBER", "")  # WhatsApp admin number
ADMIN_TG_ID     = os.getenv("ADMIN_TG_ID", "")   # Telegram admin chat ID (set this too)
...
# In-memory state (same pattern as app.py)
tg_sessions             = {}   # chat_id → session dict
tg_active_cases         = {}   # chat_id → {reporter, case_id}
tg_pending_outcome      = {}   # chat_id → {case_id, note}
tg_pending_transfer     = {}   # chat_id → {case_id, warned_at, urgency}
tg_pending_photo        = {}   # chat_id → {case_id, note, was_accepted}
tg_pending_confirm      = {}   # chat_id → {case_id, volunteer_name, vol_id, ...}
tg_waiting_reporters    = {}   # chat_id → True/False
tg_message_timestamps   = {}   # chat_id → [timestamps]


# ══════════════════════════════════════════════════════════════════
# DATABASE  (reuses same tables as app.py)
# ══════════════════════════════════════════════════════════════════
...
def load_tg_volunteers():
    """Load volunteers who registered via Telegram (phone = tg_{chat_id})"""
    conn = get_db(); cur = conn.cursor()
    cur.execute("SELECT * FROM volunteers WHERE status='active' AND phone_number LIKE 'tg_%';")
    rows = cur.fetchall(); cur.close(); conn.close()
    return {r["phone_number"]: dict(r) for r in rows}
...
def add_photo_warning(phone):
    ...
    if not row: return
    warnings = row["photo_warnings"]; name = row["name"]
    if warnings >= 5:
        conn = get_db(); cur = conn.cursor()
...
# ══════════════════════════════════════════════════════════════════
# VISION AI — PROVIDER CHAIN (Gemini → Groq → safe default)
# ══════════════════════════════════════════════════════════════════
...
def tg_id_from_phone(phone):
    """Extract chat_id from tg_{chat_id} format"""
...
# ══════════════════════════════════════════════════════════════════
# ALERT VOLUNTEERS (both WhatsApp + Telegram volunteers)
# ══════════════════════════════════════════════════════════════════
...
async def alert_all_volunteers(bot, reporter_id, session, urgency, ai_analysis, case_id):
    ...
    for vol_phone, vol_data in volunteers.items():
        ...
        if tg_id:
            # Telegram volunteer — send with button
            try:
                await send_tg(bot, tg_id, message,
            ...
            # WhatsApp volunteer — handled by app.py normally
            # We just update active_cases in DB context so WA bot can pick up
            alerted.append(vol_phone)
    ...
    threading.Timer(600, lambda: escalate_case_tg(case_id), daemon=True).start()
...
def escalate_case_tg(case_id):
    """Escalation for Telegram-reported cases"""
    case = load_case(case_id)
    if not case or case["status"] in ["ACCEPTED","COMPLETED"]:
        return
    logger.info(f"ESCALATING TG case {case_id}")
    # The WhatsApp bot's escalation handles WA volunteers
    # For TG volunteers, we'd need async context — simplified here
...
# ══════════════════════════════════════════════════════════════════
# GHOST VOLUNTEER TIMEOUT (Telegram)
# ══════════════════════════════════════════════════════════════════
...
async def reopen_stale_case_tg(bot, case_id):
    ...
    stale_tg    = tg_id_from_phone(stale_phone)
...
async def ask_next_question(bot, chat_id, session):
    """Send next question based on session state"""
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text_up.startswith("COMPLETED"):
        ...
        if len(parts) >= 2:
            ...
            await send_tg(context.bot, chat_id, "Please include Case ID.\nExample: COMPLETED CASE-XXXX")
        return
    ...
    # ── Photo stage — text received instead of photo ──
    if flow == "photo":
        await send_tg(context.bot, chat_id,
    ...
    if stage == "vol_entering_name":
        ...
        tg_sessions[chat_id] = session
        await send_tg(context.bot, chat_id, "Which city are you based in?\n\n(Type your city name)")
        return
        ...
        phone = f"tg_{chat_id}"
        save_application(phone, name, city, "community")
        session["stage"] = ""
        tg_sessions[chat_id] = session
        await send_tg(context.bot, chat_id,
            f"🐾 Thank you for applying, {name}!\n\n"
            "Your application has been received.\n\n"
            "Our team will review it and contact you for verification.\n"
            "This usually takes 1-3 days.\n\n"
            "Use /vstatus anytime to check your status.",
            keyboard=main_menu_keyboard()
        )
        if ADMIN_TG_ID:
            await send_tg(context.bot, int(ADMIN_TG_ID),
                f"🆕 NEW VOLUNTEER (Telegram)\n\nName: {name}\nCity: {city}\nTG ID: {chat_id}\n\n"
                f"To approve: send APPROVE_TG {chat_id} in admin chat"
            )
        return
...
async def handle_photo_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in tg_pending_photo:
        ...
        if cid:
            ...
            if os.path.exists(report_path):
                photo_result = compare_photos(report_path, comp_path)
            else:
    ...
    # ── Progress photo from volunteer mid-rescue ──
    volunteers = load_all_active_volunteers()
    if phone in volunteers and chat_id in tg_pending_outcome:
        ...
        if cid:
            ...
            if case:
                ...
                if reporter_tg:
                    await send_tg_photo(context.bot, reporter_tg, path, "📸 Progress photo from your volunteer")
                await send_tg(context.bot, chat_id, "✅ Progress photo shared with the reporter.")
        return

    # ── Report photo from reporter ──
    if flow != "photo":
        await send_tg(context.bot, chat_id,
    ...
    # Map severity label to number
    sev_num = session.get("severity_num", 5)

    if count_active_cases_for_reporter(phone) >= 1:
        conn = get_db(); cur = conn.cursor()
        cur.execute("SELECT case_id FROM cases WHERE reporter=%s AND status IN ('PENDING','ACCEPTED') ORDER BY time_reported DESC LIMIT 1;", (phone,))
        row = cur.fetchone(); cur.close(); conn.close()
    ...
    case = {
        "case_id": case_id, "reporter": phone,
    ...
    await send_tg(context.bot, chat_id, urgency_msg.get(urgency, urgency_msg["MEDIUM"]))
    await send_first_aid_tg(context.bot, chat_id, session)

    # Alert volunteers in background
    import asyncio
    asyncio.create_task(
        alert_all_volunteers(context.bot, chat_id, session, urgency, ai_analysis, case_id)
    )

    # Schedule photo deletion after 24 hours
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "action_report":
        ...
        await send_tg(context.bot, chat_id,
            ...
            "Your number is registered. False reports result in action.\n\n"
    ...
    if data.startswith("yn_"):
        ...
        session["flow_stage"] = get_next_flow_stage(session)
        tg_sessions[chat_id] = session
        await send_tg(context.bot, chat_id, f"✅ {key.replace('_',' ').title()}: {label_map.get(value,value)}")
        await ask_next_question(context.bot, chat_id, session)
        return
    ...
    if data == "vol_apply":
        phone  = f"tg_{chat_id}"
        status = get_volunteer_status(phone)
        if status == "active":
            await send_tg(context.bot, chat_id,
                "✅ You are already an active Anira volunteer!\n\n"
                "You will receive rescue alerts when cases come in.\n\n"
                "Commands:\n• Reply RESPONDING CASE-XXXX to accept a case\n"
        ...
        if status == "pending":
            await send_tg(context.bot, chat_id,
                ...
                "We'll contact you for verification. Usually 1-3 days."
            )
            return
    ...
    if data.startswith("vol_accept_"):
        ...
        if case["status"] == "COMPLETED":
            ...
            return
        if case["status"] == "ACCEPTED" and case.get("volunteer_number") != phone:
            await send_tg(context.bot, chat_id,
                f"Case {case_id} has already been accepted by another volunteer."
            )
            return

        vol_name = volunteers[phone]["name"]
        case["status"]         = "ACCEPTED"
        case["volunteer"]      = vol_name
        case["volunteer_number"] = phone
        ...
        await send_tg(context.bot, chat_id,
            ...
            f"When done, send: COMPLETED {case_id}\n\n"
...
# ══════════════════════════════════════════════════════════════════
# STATUS / COMPLETED / RESPONDING HELPERS
# ══════════════════════════════════════════════════════════════════
...
async def handle_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    phone   = f"tg_{chat_id}"
    conn    = get_db(); cur = conn.cursor()
    cur.execute("SELECT case_id FROM cases WHERE reporter=%s ORDER BY time_reported DESC LIMIT 1;", (phone,))
    row = cur.fetchone(); cur.close(); conn.close()
    if not row:
        await send_tg(context.bot, chat_id,
...
async def handle_completed_command(chat_id, case_id, bot):
    ...
    if case["status"] == "COMPLETED":
        ...
        return
    if case.get("volunteer_number") != phone:
        await send_tg(bot, chat_id,
    ...
    threading.Timer(1800,
        lambda: context_photo_deadline(chat_id, case_id)
    ).start()
...
async def handle_responding_text(chat_id, case_id_provided, bot):
    ...
    if case_id_provided:
        ...
        if not case:
            ...
            return
        # Reuse vol_accept logic via callback simulation
        # Inline: accept the case directly
        if case["status"] == "COMPLETED":
...
def main():
    ...
    application = (
        ...
        .concurrent_updates(True)   # handles 50+ users simultaneously
        .build()
    )
