...
def escalate_case(case_id):
    ...
    if remaining:
        for vol_number in remaining:
            ...
            active_cases[vol_number] = {"reporter": case["reporter"], "case_id": case_id}
...
def start_escalation_timer(case_id, delay_seconds=600):
    ...
    timer.daemon = True
    timer.start()
    print(f"Escalation timer started for {case_id}")
...
def send_first_aid(sender, session):
    ...
    if severity >= 7:
        urgency_note = "⚠️ This is a serious case. Please stay calm and do not move the animal unless absolutely necessary.\n\n"
    else:
        urgency_note = ""
    ...
    if bleeding == "YES":
        tips.append("🩸 If bleeding: Apply gentle pressure with a clean cloth. Do not remove it.")
    if can_move == "NO":
        tips.append("🚫 Do not try to lift or drag the animal — this can cause more injury.")

    if animal == "dog":
        tips.append("🐕 Keep people and other animals away to reduce stress.")
        tips.append("💧 Do not force feed water — only offer if the dog is conscious and calm.")
    elif animal == "cat":
        tips.append("🐈 Cats hide pain — keep very still and quiet near them.")
        tips.append("🧤 Use a cloth or jacket to gently cover the cat — it reduces panic.")
    elif animal == "cow":
        tips.append("🐄 Keep people away — crowd stress can worsen condition.")
        tips.append("☀️ If in sun, try to shade the animal if possible.")
    elif animal == "bird":
        tips.append("🐦 Place a cloth loosely over the bird to keep it calm.")
        tips.append("🌡️ Keep it warm — birds go into shock quickly.")
    else:
        tips.append("🐾 Keep the animal calm and stay at a safe distance.")
        tips.append("👥 Ask bystanders to move away — crowds increase stress.")

    tips.append("📵 Keep noise levels low around the animal.")
    tips.append("🚫 Do not give any food or medicine without vet advice.")

    tip_text = "\n".join(tips)

    send_message(sender,
        f"🐾 While you wait — First Aid Tips:\n\n"
        f"{urgency_note}"
        f"{tip_text}\n\n"
...
def handle_responding(sender, volunteer_name, case_data):
    ...
    # Ask volunteer for outcome note while heading to rescue
    pending_outcome[sender] = {"case_id": case_id_found, "note": None}
    send_message(sender,
        "📝 While you're heading there — please send a brief note about the outcome once rescue is done:\n\n"
        "Example:\n"
        "\"Taken to vet on MG Road, stable\"\n"
        "\"Given water, recovering on site\"\n"
        "\"Animal had already passed\"\n\n"
        "Send your note anytime, then type:\n"
...
def handle_outcome_note(sender, text):
    ...
    send_message(sender,
        ...
        f"COMPLETED {data['case_id']}"
    )
    print(f"Outcome note saved for {data['case_id']}: {text.strip()}")
    return True
...
def handle_status(sender, text):
    ...
    if case.get("time_accepted"):
        msg += f"Accepted: {case['time_accepted']}\n"
    if case.get("time_completed"):
        msg += f"Completed: {case['time_completed']}\n"
    if case.get("outcome"):
...
def handle_completed(sender, text):
    ...
    if case["status"] == "COMPLETED":
        send_message(sender, f"Case {case_id} is already completed.")
        return

    # Get saved note if exists
    outcome_data = pending_outcome.get(sender, {})
    note = outcome_data.get("note", None) if isinstance(outcome_data, dict) else None
    ...
    if stage == "warning":
        ...
        if pending_key:
            ...
            if interpreted == "UNCLEAR":
                ...
                if unclear_count >= 3 and severity >= 4:
                    ...
                    qtype, question = ALL_QUESTION_MAP.get(
                        pending_key, ("text", "Could you please clarify your answer?")
                    )
                    send_message(sender, f"Sorry, I didn't quite understand.\n\n{question}")
                return
            session["unclear_count"] = 0
            if pending_key == "animal":
                ...
                session["animal"] = animal_map.get(interpreted.lower(), interpreted.capitalize())
        ...
        if text.upper() == "STAY" or interpreted == "YES":
            ...
            for vol_num, case_data in list(pending_volunteer_responses.items()):
                if isinstance(case_data, dict) and case_data.get("reporter") == sender:
                    ...
                    volunteers = load_volunteers()
                    volunteer_name_waiting = volunteers.get(vol_num, {}).get("name", "Volunteer")
                    break
            if volunteer_waiting:
                connect_reporter_volunteer(sender, volunteer_waiting, volunteer_name_waiting)
            else:
                send_message(sender,
                    "🙏 Thank you for staying.\n"
                    "You are making a real difference.\n\n"
...
def alert_volunteers(sender, session, urgency, gemini_analysis, case_id):
    ...
    for vol_number in volunteers:
        ...
        active_cases[vol_number] = {"reporter": sender, "case_id": case_id}
        alerted.append(vol_number)
        ...
        if "messages" in value:
            ...
            if message["type"] == "text":
                ...
                if session.get("stage") != "location":
                    send_message(sender, "Please complete the questions first before sharing location.")
                    return "OK", 200
                ...
                if sender in volunteers and sender in pending_outcome:
                    outcome_data = pending_outcome.get(sender, {})
                    case_id = outcome_data.get("case_id") if isinstance(outcome_data, dict) else None
                    if case_id:
                        completion_photo_path = f"completion_{case_id}.jpg"
                        image_id = message["image"]["id"]
                        image_url = get_image_url(image_id)
                        download_image(image_url, completion_photo_path)
                        cases = load_cases()
                        if case_id in cases:
                            reporter = cases[case_id]["reporter"]
                            upload_and_send_photo(reporter, completion_photo_path,
                                "📸 Photo from rescue volunteer after completing rescue")
                            cases[case_id]["completion_photo"] = True
                            save_cases(cases)
                            send_message(sender, "✅ Photo shared with reporter. Thank you!")
                        return "OK", 200

                # ── IGNORE OTHER VOLUNTEER IMAGES ──
                ...
                if session.get("stage") != "photo":
                    send_message(sender, "Please answer all questions first before sending a photo.")
                    return "OK", 200

                send_message(sender, "📸 Photo received. Sending details to rescue team...")
                ...
                if not found_animals:
                    send_message(sender, "❓ No animal detected in the image.\nPlease send a clearer photo.")
                    return "OK", 200
